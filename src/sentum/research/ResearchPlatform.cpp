...
ResearchSummary ResearchRunner::run(const ResearchConfig& input) const {
    ...
    ResearchSummary out;out.dataset=c.dataset;out.symbol=c.symbol;out.objective=c.objective;out.generated_at_ms=std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now().time_since_epoch()).count();out.events=events.size();out.research_events=research_end;out.holdout_events=holdout_n;out.initial_train_events=initial_train;out.holdout_begin_index=research_end;out.purge_events=c.purge_events;out.embargo_events=c.embargo_events;out.folds=folds;out.trials=trial_count;out.results.resize(trial_count);
    out.fold_boundaries.reserve(folds);
    for(std::size_t f=0;f<folds;++f){const std::size_t boundary=initial_train+f*fold_width,ve=f+1==folds?research_end:std::min(research_end,initial_train+(f+1)*fold_width),te=boundary>c.purge_events?boundary-c.purge_events:0,vb=std::min(ve,boundary+c.embargo_events);out.fold_boundaries.push_back({f,0,te,vb,ve,boundary-te,vb-boundary});}
    ...
    for(std::size_t w=0;w<workers;++w)pool.emplace_back([&]{while(true){const std::size_t idx=next.fetch_add(1);if(idx>=ps.size())break;const auto p=ps[idx];std::vector<TradePosition> train,oos;for(const auto& fold:out.fold_boundaries){if(fold.train_end_exclusive){auto r=run_slice(events,fold.train_begin,fold.train_end_exclusive,p,base_risk_,c.symbol,false);train.insert(train.end(),r.trades.begin(),r.trades.end());}if(fold.validation_begin<fold.validation_end_exclusive){auto r=run_slice(events,fold.validation_begin,fold.validation_end_exclusive,p,base_risk_,c.symbol,true);oos.insert(oos.end(),r.trades.begin(),r.trades.end());}}TrialResult r;r.trial_id=idx+1;r.parameters=p;r.train=MetricsCalculator::calculate(train);r.validation=MetricsCalculator::calculate(oos);r.train_score=score(r.train,c.objective);r.validation_score=score(r.validation,c.objective);r.overfit_gap=r.train_score-r.validation_score;r.deflated_sharpe=deflated_sharpe(r.validation.sharpe,r.validation.trades,trial_count);r.eligible=r.validation.trades>=c.min_validation_trades;out.results[idx]=std::move(r);}});for(auto&t:pool)t.join();
...
nlohmann::json ResearchRunner::to_json(const ResearchSummary&s){nlohmann::json j{{"dataset",s.dataset},{"symbol",s.symbol},{"objective",s.objective},{"generated_at_ms",s.generated_at_ms},{"events",s.events},{"research_events",s.research_events},{"holdout_events",s.holdout_events},{"folds",s.folds},{"trials",s.trials},{"holdout_evaluated",s.holdout_evaluated},{"leaderboard",nlohmann::json::array()}};
    nlohmann::json validation_contract={{"schema_version",1},{"initial_train_events",s.initial_train_events},{"research_end_index",s.research_events},{"holdout_begin_index",s.holdout_begin_index},{"holdout_end_index",s.events},{"purge_events",s.purge_events},{"embargo_events",s.embargo_events},{"fold_boundaries",nlohmann::json::array()}};
    for(const auto&fold:s.fold_boundaries)validation_contract["fold_boundaries"].push_back({{"fold_index",fold.fold_index},{"train_begin",fold.train_begin},{"train_end_exclusive",fold.train_end_exclusive},{"validation_begin",fold.validation_begin},{"validation_end_exclusive",fold.validation_end_exclusive},{"purged_events",fold.purged_events},{"embargoed_events",fold.embargoed_events}});
    j["validation_contract"]=std::move(validation_contract);for(const auto&t:s.leaderboard)j["leaderboard"].push_back(trial_json(t));if(s.holdout_evaluated){j["selected_parameters"]=parameter_json(s.selected_parameters);j["final_holdout"]=metrics_json(s.final_holdout);j["final_holdout_score"]=finite_or_zero(s.final_holdout_score);j["bootstrap_net_profit"]=interval_json(s.bootstrap_net_profit);j["monte_carlo"]={{"samples",s.monte_carlo.samples},{"net_profit",interval_json(s.monte_carlo.net_profit)},{"max_drawdown",interval_json(s.monte_carlo.max_drawdown)},{"probability_of_loss",finite_or_zero(s.monte_carlo.probability_of_loss)}};j["holdout_regimes"]=nlohmann::json::array();for(const auto&r:s.holdout_regimes)j["holdout_regimes"].push_back({{"regime",r.regime},{"metrics",metrics_json(r.metrics)}});}return j;}
